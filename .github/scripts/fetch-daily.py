...
def mega_batch_summarize(pending: list[tuple], api_key: str, cache: dict) -> int:
    """全セクション横断の未要約 items を 1 リクエストで要約。
    pending は [(item, requirement_text), ...]。
    1 リクエストにまとめることで Gemini Free のレート制限から逃げる。
    """
    if not pending or not api_key:
        return 0
    bullets = []
    for i, (it, req) in enumerate(pending, 1):
        title = it.get('title', '')
        body = it.get('body', '') or '(本文なし)'
        bullets.append(
            f'### 項目 {i}\n要件: {req}\nタイトル: {title}\n本文: {body}'
    ...
    full = (
        'あなたは日本語要約者です。以下の項目それぞれを、各項目に書かれた要件に従い'
        '日本語で要約してください。元の情報にないことは書かないでください。\n\n'
        + '\n\n'.join(bullets)
        + (
            f'\n\n上記 {len(pending)} 件すべてについて、項目1から順に対応する'
            ' 日本語要約文字列を JSON 配列で返してください。'
    ...
    try:
        ...
        if not isinstance(result, list):
            return 0
        n = 0
        for (it, _), s in zip(pending, result):
            if isinstance(s, str) and s.strip():
                it['summary'] = s.strip()
                cache[it['url']] = s.strip()
                n += 1
        return n
    except Exception as e:
        print(f'[gemini mega batch] parse failed: {e}')
        return 0
...
def main(out_path: str, archive_dir: str | None = None) -> None:
    ...
    sections = [
        (news,         'news',         '200〜260字の日本語要約。タイトル/冒頭にない情報は付け加えない'),
        (medical_news, 'medical',      '200〜260字の日本語要約'),
        (tech,         'tech',         '200〜260字の日本語要約'),
        (papers_mgmt,  'papers_mgmt',  '英語の abstract を 250〜300字の日本語要約。研究の問い・方法・結果・含意を含める'),
        (papers_med,   'papers_med',   '英語の abstract を 250〜300字の日本語要約'),
        (arxiv_papers, 'arxiv',        '英語の abstract を 250〜300字の日本語要約'),
        (wiki,         'wiki',         '英語のできごとを 180〜240字の日本語要約。馴染みの薄い名称には簡単な補足'),
    ]

    # まず cache 復元
    for items, label, _ in sections:
        for it in items:
            u = it.get('url', '')
            if u and u in cache:
                it['summary'] = cache[u]

    # 全セクション横断で uncached なものを集めて 1 リクエストでまとめて要約
    pending: list[tuple] = []  # (item, requirement)
    for items, label, req in sections:
        for it in items:
            if it.get('url') and not it.get('summary'):
                pending.append((it, req))
